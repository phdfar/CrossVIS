...
from adet.utils.comm import aligned_bilinear, compute_locations
...
def compute_pairwise_term(mask_logits, pairwise_size, pairwise_dilation):
    ...
    log_fg_prob_unfold = unfold_wo_center(log_fg_prob,
                                          kernel_size=pairwise_size,
                                          dilation=pairwise_dilation)
    log_bg_prob_unfold = unfold_wo_center(log_bg_prob,
                                          kernel_size=pairwise_size,
                                          dilation=pairwise_dilation)
...
def dice_coefficient_attention(x, target):
    atten_map = F.softmax(x, dim=-1)
    eps = 1e-5
    n_inst = x.size(0)
    x = x.reshape(n_inst, -1)
    target = target.reshape(n_inst, -1)
    atten_map = atten_map.reshape(n_inst, -1)

    # Apply attention weights to predicted mask
    x = x * atten_map
...
class DynamicMaskHead(nn.Module):
    ...
    def pdice(self,pred_mask,gt_mask):
      # assume your tensors are named pred_mask and gt_mask
      patch_size = 16
      batch_size = pred_mask.size(0)
      num_patches = (pred_mask.size(2) // patch_size) * (pred_mask.size(3) // patch_size)
      patch_channels = pred_mask.size(1)
      patch_height = patch_size
      patch_width = patch_size

      # reshape tensors to patches
      pred_patches = pred_mask.unfold(2, patch_size, patch_size).unfold(3, patch_size, patch_size).contiguous().view(batch_size, num_patches, patch_channels, patch_height, patch_width).transpose(0, 1)
      gt_patches = gt_mask.unfold(2, patch_size, patch_size).unfold(3, patch_size, patch_size).contiguous().view(batch_size, num_patches, patch_channels, patch_height, patch_width).transpose(0, 1)

      # calculate loss for each patch
      loss = 0
      for i in range(num_patches):
          loss += dice_coefficient_attention(pred_patches[i], gt_patches[i]).mean()
            ...
            if len(pred_instances) == 0:
                ...
                if self.boxinst_enabled:
                    ...
                    image_color_similarity = image_color_similarity[
                        gt_inds].to(dtype=mask_feats.dtype)
                    ...
                    weights = (image_color_similarity >=
                               self.pairwise_color_thresh
                               ).float() * gt_bitmasks.float()
                    loss_pairwise = (pairwise_losses * weights
                    ...
                    ploss = self.pdice(mask_scores,gt_bitmasks)
                    loss_mask = mask_losses.mean()
                    #print('mask_scores',mask_scores.size())
                    #print('ploss',ploss)
                    #print('loss_mask',loss_mask)
                    losses['loss_mask'] = 0.5*loss_mask + 0.5*ploss
