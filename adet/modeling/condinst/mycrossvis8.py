...
def total_gt(gt0,gt1):
  gt_final=[]
  for i in range(len(gt1)):
    a = gt0[i].id ; b = gt1[i].id 
    z = torch.cat((a,b))
    t = torch.unique(z)
    ax = gt0[i].gt_bitmasks_full.long()
    bx = gt1[i].gt_bitmasks_full.long()
    tempa = torch.zeros((t.size(0),ax.size(1),ax.size(2)))
    tempb = torch.zeros((t.size(0),bx.size(1),bx.size(2)))
    for j,ts in enumerate(t):
      if ts in a:
        index = torch.where(ts==a)[0]
        tempa[j]=ax[index]
      if ts in b:
        index = torch.where(ts==b)[0]
        tempb[j]=bx[index]
    x = tempa.unsqueeze(1)
    y = tempb.unsqueeze(1)
...
class CrossVIS(nn.Module):
    ...
    def emd_func(self,xmask_feats_1,gt_final):
        ...
        for j in range(0,16):
          embeddings_per_seq = xmask_feats_1[j].permute(1,2,0)  # [N, T, H, W, C]
    ...
    def forward_train(self, batched_inputs):
        ...
        #print('z',torch.sum(x,dim=0).size())
        ...
        losses.update({'loss_embedding': lovasz_loss_big})
