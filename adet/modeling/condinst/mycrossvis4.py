...
class CrossVIS(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.cbam = CBAM(8,reduction_ratio=2)
        ...
        self.fskey_conv = nn.Conv2d(8,8,3,padding='same')
        self.fsvalue_conv = nn.Conv2d(8,8,3,padding='same')
        self.fckey_conv = nn.Conv2d(8,8,3,padding='same')
        self.softmax_attn = nn.Softmax(dim=1)
        self.fckey_conv = nn.Conv2d(8,8,3,padding='same')
        self.fA_conv  = nn.Conv2d(8,8,1,padding='same')
    ...
    def forward_train(self, batched_inputs):
        ...
        z = torch.cat((x,y),dim=2) # [N C T H W]
        ...
        fcbams0 = self.cbam(mask_feats_0)
        fcbams1 = self.cbam(mask_feats_1)

        mask_feats_0 = ff[:,:,0,:,:] + fcbams0
        mask_feats_1 = ff[:,:,1,:,:] + fcbams1
        ...
        mask_feats_0 = mask_feats_0 + z
        mask_feats_1 = mask_feats_1 + z
        """
