...
from .Optical_Flow_Backbone import backbone
...
class CrossVIS(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.backbone = backbone.RAFTTransformerBackbone()
    ...
    def forward_train(self, batched_inputs):
        ...
        images_norm_0 = ImageList.from_tensors(images_norm_0,32)

        images_norm_1 = [self.normalizer(x) for x in original_images_1]
        images_norm_1 = ImageList.from_tensors(images_norm_1,32)

        #features_0_origin = self.backbone(images_norm_0.tensor)
        #features_1_origin = self.backbone(images_norm_1.tensor)
        
        self.backbone.eval()
        features_0_origin = self.backbone(images_norm_0.tensor,images_norm_0.tensor)
        features_1_origin = self.backbone(images_norm_1.tensor,images_norm_1.tensor)
        

        p40 = torch.nn.functional.interpolate(features_0_origin, size=(features_0_origin.size(2)//2, features_0_origin.size(3)//2), mode='bilinear', align_corners=False)
        p41 = torch.nn.functional.interpolate(features_1_origin, size=(features_0_origin.size(2)//2, features_0_origin.size(3)//2), mode='bilinear', align_corners=False)
        
        p50 = torch.nn.functional.interpolate(features_0_origin, size=(features_0_origin.size(2)//4, features_0_origin.size(3)//4), mode='bilinear', align_corners=False)
        p51 = torch.nn.functional.interpolate(features_1_origin, size=(features_0_origin.size(2)//4, features_0_origin.size(3)//4), mode='bilinear', align_corners=False)
        
        p60 = torch.nn.functional.interpolate(features_0_origin, size=(features_0_origin.size(2)//8, features_0_origin.size(3)//8), mode='bilinear', align_corners=False)
        p61 = torch.nn.functional.interpolate(features_1_origin, size=(features_0_origin.size(2)//8, features_0_origin.size(3)//8), mode='bilinear', align_corners=False)
        
        p70 = torch.nn.functional.interpolate(features_0_origin, size=(features_0_origin.size(2)//16, features_0_origin.size(3)//16), mode='bilinear', align_corners=False)
        p71 = torch.nn.functional.interpolate(features_1_origin, size=(features_0_origin.size(2)//16, features_0_origin.size(3)//16), mode='bilinear', align_corners=False)
        
        
        features_0 = {'p3':features_0_origin,'p4':p40,'p5':p50,'p6':p60,'p7':p70}
        features_1 = {'p3':features_1_origin,'p4':p41,'p5':p51,'p6':p61,'p7':p71}
